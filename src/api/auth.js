...
const BASE = import.meta.env.VITE_API_BASE || 'https://api.example.com' // replace with your backend
...
export async function wxLogin(){
  ...
  return new Promise((resolve, reject)=>{
    uni.login({ provider: 'weixin', success: async (loginRes)=>{
      try{
        const res = await httpFetch('/api/auth/wx-login', { method: 'POST', body: JSON.stringify({ code: loginRes.code }) })
        const j = await res.json()
        if(!res.ok) return reject(new Error(j.message || 'wx login failed'))
        if(j.token) applySession({ user: j.user, access_token: j.token })
        resolve(j)
      }catch(e){ reject(e) }
    }, fail: reject })
